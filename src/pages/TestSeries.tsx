  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-white">Test Series Management (Paid)</h1>
          <p className="text-sm text-neutral-400">24-Hour Allen Model Schedule & Server-Enforced Preview Gate</p>
        </div>
        <button
      ...
      {loading ? (
        <div className="text-white">Loading test series papers...</div>
      ) : (
            ...
            <tbody>
              {tests.map((t) => (
                <tr key={t.id} className="border-b border-white/5 hover:bg-white/5">
                  ...
                  <td className="px-6 py-4">{t.exam_type || 'IAT'}</td>
                  <td className="px-6 py-4 text-xs font-mono">{formatDate(t.window_start)}</td>
                  <td className="px-6 py-4 text-xs font-mono">{formatDate(t.window_end)}</td>
                  <td className="px-6 py-4">{t.duration_minutes || 180} mins</td>
              ...
              {tests.length === 0 && (
                <tr><td colSpan={8} className="p-4 text-center">No scheduled test series papers found.</td></tr>
              )}
            </tbody>
          </table>
        </div>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center p-4 z-50">
          <div className="bg-neutral-900 border border-white/10 rounded-xl w-full max-w-md p-6">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-xl font-bold text-white">Create Test Series Paper</h2>
            ...
            <form onSubmit={handleCreateTest} className="space-y-4">
              ...
              <div>
                <label className="block text-sm text-neutral-400 mb-1">Window Open Time (Start)</label>
              ...
              <div>
                <label className="block text-sm text-neutral-400 mb-1">Window Close Time (End)</label>
